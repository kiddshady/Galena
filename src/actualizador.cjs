...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   levantar la app. Portado de Tessera (y antes de Quire), donde está probado de punta a punta.
   ═══════════════════════════════════════════════════════════════════════════ */
...
const RELEASES = 'https://github.com/kiddshady/Galena/releases';
...
function soporte({ empaquetada = false, portable = false } = {}) {
  if (!empaquetada) {
    return { ok: false, motivo: 'Estás corriendo Galena desde el código fuente. Acá no hay nada que actualizar: usá git.' };
  }
  if (portable) {
...
function iniciar(getWin, {
  ...
  /* Si nunca hacés click en "reiniciar", la actualización entra igual la próxima
     vez que cerrás Galena. Es la parte que hace que esto sirva de verdad. */
  ...
  autoUpdater.on('update-available', (info) => fijar({
    ...
    nombre: String(info?.releaseName || '').trim() || `Galena ${info?.version || ''}`.trim(),
    bytes: tamanoDe(info),
